...
- [x] 1. Create `LoadingSpinner` UI component
...
- [x] 2. Update `Button` UI component for inline loading
...
- [x] 3. Implement Loading State in `Login` Page
...
- [x] 4. Implement Loading State in `ListOrder` Page
...
- [x] 5. Implement Loading State in `DetailOrder` Page
...
- [x] 6. Implement Loading State in `CreateOrder` Page
...
- [x] 7. Implement Menu Fetching Loading State in `CreateOrder` Page
  - Add `isFetchingMenu` state to `src/components/pages/CreateOrder/CreateOrder.tsx`
  - Wrap `getMenus` API call inside `useEffect` in a `try/finally` block
  - Conditionally render `<LoadingSpinner centered />` instead of the menu list grid when fetching
  - Requirements: 5
